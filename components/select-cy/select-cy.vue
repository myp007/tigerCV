<template>
  <view class="uni-select-cy" :style="{ 'z-index': zindex }">
    ...
    <view
      ...
      v-if="active && realValue.length >= maxlength"
      >Select maximum of {{ maxlength }}</view
    >
    <!-- 下拉选项 -->
    <scroll-view
      class="uni-select-cy-options"
      :scroll-y="true"
      v-show="active"
  ...
  methods: {
    ...
    handleChange(index, item) {
      ...
      if (arrIndex > -1) {
        ...
        if (this.realValue.length >= this.maxlength) {
          return;
        }
