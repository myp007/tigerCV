	...
	<view style="background-color: #fff;position: relative;overflow: hidden;" >
		...
		<view class="im-write" id="im-write" :style="{'bottom': writeBottom  + 'px'}">
			<view v-if="sessionUserInputStatus" class="session-user-input-status-box">
				<view :style="{bottom: (imWriteHeight / 2) + 'px'}" class="session-user-input-status">The other is typing...</view>
			</view>
			...
			<view class="write-right" :style="{flex:showSendButton ? 3:2}">
				...
				<!-- <button class="send-btn"  hover-class="send-btn-hover" >
					SEND
				</button> -->
				<image class="send-icon" src="@/static/icon/send-icon.png" @click="sendMessage(imMessage, 'default')" v-if="showSendButton"></image>
				<image class="toolbar-icon more" src="/static/icon/more.png" @click="clickMore('more')" mode="widthFix" v-if="!showSendButton"></image>
			</view>
		</view>
		<!-- 消息输入-end -->
