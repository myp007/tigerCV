<template>
	<view class="tiger_cv_index" id="main_body">
		...
		<view class="jobgr" >
			<view class="job_ct">
				<view class="topClass">
                    <view class="topItem" :class="{ 'topactive': i.id == directoryInfo.id }" @click="topTap(i)"
                        v-for="(i, index) in topPosition" :key="index">
                        {{ i.name }}
                        <block v-if="i.count > 0">({{ i.count || 0 }})</block>
                    </view>
                </view>
				<view class="backyingyin"></view>
			</view>
		</view>
		<!-- content -->
		<view class="content_box job_list" :style="{ 'padding-top':  topPosition.length >3 ? '30px' : '0px'}">
			<u-skeleton v-if="showSkeleton" class="p_10 mt_10" :loading="true" rows="3" v-for="(item,i) in 4" :key="i"
				:rowsHeight="[10, 10, 30]"></u-skeleton>
			...
			<u-list v-if="!showSkeleton" @scrolltolower="scrolltolower"  @scroll="scrollPosition"
				class=" card_item white_bj mb_10" style="height: 100%;" :preLoadScreen="listData.length">
				<view class="results">
					...
					<span v-if="queryData.keyWord.length > 0"> Search Criteria:<span v-for="(k, i) in queryData.keyWord">{{
						k.value }}</span></span>
						<!-- //v-if="queryData.job_no || queryData.location || queryData.keyWord.length > 0" -->
					<span  class="reset" v-if="positionInfo.id"
						@click="resetSearch"> Reset</span>  
				</view>
				<u-list-item v-for="(item, index) in listData" :key="index" class="job_list_item">
					<view
						style="background-color: #fff;padding: 20px;border-bottom: 1px solid #f2f2f2;margin-bottom: 10px;">
						<view class="title">
							{{ item.jobname || '-' }}
						</view>
						<view class="time">
							<view class="grey_text" style="color: #00328f;" @click="seeMore(index)">
								{{ item.companyname }}
							</view>
						</view>
					...
					<view style="margin-bottom: 10px;padding: 0 22px 0 20px;display: flex;align-items: center;">
						...
						<view class="publisher_date" style="width: 50%;color:#999">
                            ...
                            <image src="@/static/icon/chat_icon.png" class="chat_icon" mode="aspectFit"  @click="goChat(item)"
						...
						<view 
							...
							<!-- <view  class="line_feed" @tap="goChat(item)">
								<image :src="item.avatar || defaultAvatar" class="avatar" mode="aspectFit"
									lazy-load="false"></image>
								<view class=" fz_12  tr chat_but" style="color: #00328f;text-align: left;margin-left:10px">Chat Now</view>
							</view> -->
							<!-- <u-icon name="chat-fill" size="24" color="#00328f" v-if="item.is_chat > 0" @click="goChat(item)"></u-icon> -->
						</view>
					</view>
		...
		<view class="dropdown_abs" v-if="dropdownShow" :style="{ 'top':  topPosition.length >3 ? '110px' : '80px'}">
			<scroll-view scroll-y="true" style="height:calc(100% - 150px);" class="scroll">
				<view class="dropdown_item" :class="{ 'dropdown_active': item.jobcode == positionInfo.jobcode }"
					v-for="(item, index) in listPosition" :key="index" @click="dropdownChang(item)">
					{{ item.jobname }}
		...
		<view class="applybox" v-if="applyShow">
			<u-icon class="leftIcon" name="arrow-left" size="20" @click="closeApply"></u-icon>
			...
			<view class="apply_info" v-if="showSucces">
				...
				<view class="success_text">
					...
					<view class="text2">Taking the initiative to communicate directly with HR, will greatly increase the other party's attention to you.</view>
				</view>
				<scroll-view scroll-y="true" class="scroll-y">
					<view class="avatar_list">
					<view style="width:15px"></view>
					<view class="avatar_box" :style="{'z-index': i+1}" v-for="(item,i) in checkList" :key="i" @click="topAvatar(item)" v-if="i < 8">
						<image   :src="item.avatar || defaultAvatar" mode="aspectFill" class="avatar"></image>
					</view>
				</view>
				</scroll-view>
				
				<view class="" style="margin-top:20px;text-align: center;">Chat with HR right away.</view>
				<view class="but-box" style="margin-top:20px">
					
					<u-button class="but" @click="closeSuccess" :loading="deliverLoading">Start Conversation</u-button>
				</view>
			</view>
			<view class="apply_info" v-else>
				...
				<scroll-view scroll-y="true" class="scroll-y">
					<view class="apply_item" v-for="(item, i ) in checkList" :key="i">
						...
						<view class="right_but" @click="applyRemove(i)"><u-icon class="l" name="trash" color="#00328f" size="24"></u-icon></view>
                        
					</view>
                    <view v-if="isLogin">
                        <view class="job_tit h" style="margin-top:20px;">Select resume </view>
                        <uni-data-select class="form_select" v-model="deliverForm.rid" :value="deliverForm.rid" placeholder=" "
                        ...
                        <view class="but-box">
                            <u-button class="but" @click="jobDelivery" :loading="deliverLoading">Submit & Chat Now</u-button>
                        </view>
                    </view>
                    <view v-else>
                        <view class="h" style="margin:20px 0px;">Your application : <span class="but-text">(Required)</span></view>
                        <u-form :model="formJob" ref="uForm">
                            ...
                            <u-form-item label="" :required="true" :border-bottom="true" label-width="0">
                                <span class="label_name"> Upload you CV :</span>
                                <uni-file-picker mode="list" :sizeType="['compressed']"  @select="select" @delete="deleteFile" :auto-upload="false" return-type="object" :sourceType="['album']" 
                                    v-model="filePDF" file-mediatype="all">
                                    <view slot="default">
                        ...
                        <view class="but-box">
                            <view class="but-text">
                                By clicking the "Submit & Chat now" button, you agree to our <span class="link_url" @click="openUrl('1')">User Terms</span>  and have read and acknowledge the 
                                <span class="link_url" @click="openUrl('2')">Privacy Policy Statement</span>
                            </view>
                            <u-button class="but" @click="jobSubmit" :loading="deliverLoading">Submit & Chat Now</u-button>
                        </view>
                    </view>
				</scroll-view>
		...
		<view class="v_card p_0_10 btm_form_view" v-show="showDeliver" style="padding-bottom: 20px;">
			<u-icon class="closeIcon" name="close" size="20" @click="showDeliver = false; eailmIndex = ''"></u-icon>

			<view style="max-height: 300px;overflow: auto; margin-top: 15px;">
				<u-tag class="dib" v-for="(name, index) in deliverForm.hr_email" :key="index" :text="name" size="mini" :show="true"
					@close="closeHremail(index)"></u-tag>
			</view>
			<u-row>
				<u-col span="11">
					<u--form>
						<u-form-item borderBottom @click="showPickResume = true;">
							<view style="max-height: 300px;overflow: auto;padding-left: 10px;">
								<view>{{ deliverForm.resumeName ? deliverForm.resumeName : 'your Resume *' }}</view>
							</view>
				...
				<u-col span="1">
					<u-icon class="pl_10" name="rewind-right-fill" size="20"
						@click="$u.debounce(deliverFromSubmit('send'), 500)"></u-icon>
				</u-col>
			</u-row>
		</view>
		<multiple-select v-model="showEmail" :data="emailActions" :default-selected="deliverForm.hr_email"
			@confirm="(v) => { confirm(v, 'group') }"></multiple-select>
		<u-action-sheet title="select resume" :actions="pickResume" :closeOnClickOverlay="true" :closeOnClickAction="true"
			:show="showPickResume" @close="showPickResume = false"
			@select="(o) => { onSelectChange(o.value) }"></u-action-sheet>

	</view>
</template>

<script>
import { getGroupCount, facebookChangePos, posDeliverBySelf, getChat,chatStart, getJobCategory, jobList, getTopdirectory } from '@/api/facebookTool';
...
export default {
	...
	onLoad() {
		...
		// let emailUrl = uni.getStorageSync('emailUrl');
		// let groups_name = uni.getStorageSync('groups_name');
		// let groups_id = uni.getStorageSync('groups_id')
		// if (queryData) {
		// 	this.searchParams.location = queryData.location
		// 	this.searchParams.job_no = queryData.job_no
		// 	this.queryData = queryData
		// }
		// if (emailUrl) {
		// 	this.searchParams.hr_email = emailUrl
		// }
		// if (groups_name) {
		// 	this.searchParams.groups_name = groups_name
		// }
		// if (groups_id && groups_name) {
		// 	this.searchGroup(groups_name, groups_id)
		// }
	...
	methods: {
		...
		applyRemove(index) {
			...
			// 	uni.$u.toast('Please select the position to be delivered')s
			// 	return
			// }
		...
		getListData(type) {
			...
			this.searchParams.keyWord = this.queryData.keyWord.map(o => { return o.value }).join(',')
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState === 4 && xhr.status === 200) {
					that.checkList.map((item) => {
                            that.$set(that.listData[item.index], 'self_count', ((that.listData[item.index].self_count * 1) + 1))
                            that.$set(that.listData[item.index], 'is_chat', '1')
                    })
                    uni.hideLoading();
		...
		jobDelivery() {
			...
			sendEmail(jsonData).then(res => {
				...
				this.checkList.map((item) => {
						this.$set(this.listData[item.index], 'self_count', ((this.listData[item.index].self_count * 1) + 1))
						this.$set(this.listData[item.index], 'is_chat', '1')
					})
